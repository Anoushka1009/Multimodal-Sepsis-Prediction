...
def _load_dictionary(extracted_dir: str | Path, filename: str) -> pd.DataFrame:
    path = Path(extracted_dir) / filename
    if not path.exists():
        return pd.DataFrame(columns=["ITEMID", "LABEL"])
    df = pd.read_csv(path, usecols=lambda c: c.upper() in {"ITEMID", "LABEL"}, low_memory=False)
...
def detect_culture_orders(extracted_dir: str | Path, low_memory: bool = True) -> pd.DataFrame:
    cultures = load_table(
        ...
        usecols=["SUBJECT_ID", "HADM_ID", "CHARTTIME", "SPEC_TYPE_DESC", "ORG_NAME", "TEST_NAME"],
        low_memory=low_memory,
    )
    cultures["CHARTTIME"] = pd.to_datetime(cultures["CHARTTIME"], errors="coerce")
    cultures = cultures.dropna(subset=["HADM_ID", "CHARTTIME"]).copy()
    cultures = cultures.rename(columns={"CHARTTIME": "culture_time"})
