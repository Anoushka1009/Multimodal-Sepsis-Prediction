...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train text-augmented tabular multimodal baselines locally.")
    ...
    parser.add_argument("--device", default=None, help="Device override for text embedding extraction: auto, cuda, or cpu.")
...
def main() -> None:
    ...
    training_output = train_tabular_multimodal_models(
    ...
    output_dir = paths["processed_data_dir"] / config.get("tabular_multimodal", {}).get("output_stage", "06_baseline_models")
    saved_paths = save_dataframe_bundle(training_output["artifacts"], output_dir)

    manifest_path = paths["manifests_dir"] / f"06_tabular_multimodal_{dataset_name}_manifest.json"
    write_run_manifest(
        path=manifest_path,
        stage="06_tabular_multimodal",
        config=config,
        extra={
            "dataset_name": dataset_name,
    ...
    print(f"Saved tabular multimodal artifacts to: {output_dir}")
